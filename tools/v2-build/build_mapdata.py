...
# ⚠ 2026-08-08 全件撤去。ここに店名を足さないこと。
#
# 名前が置けない店を減らす目的で、地図の上だけ短い呼び名にする表を作り、29件まで増やした。
# 組合の担当者(あみさん)から「だんだん名前が省略されてきてる」と指摘を受けて実測したところ、
# 29件を全部やめて正式名称に戻しても、名前が出る店の数は1件も変わらなかった。
#   既定表示 390x844: 22件中17件 (短縮あり・なしとも同じ)
#   小型 360x640:     14件中11件 (同じ)
#   縮尺1.45〜3.00を地図全体で舐めて名前が出た店: 58/60 (同じ)
# 効いていたのは spreadMarks (印の押し分け) / retryBlocked (席の譲り合い) /
# 佐藤次夫税理士事務所の位置修正 の方で、短縮は昔の対策の残骸だった。
#
# 店名は組合が出す情報。表示の都合で勝手に縮めない。
# どうしても必要になった時は、組合の確認を取ってからにすること。
MAP_LABEL_SHORT = {}
OFFICIAL_URL_FIX = {
...
# 公式サイトの取り込みは 2026-08-04、その上に組合(あみさん)の訂正9件を 2026-08-14 に重ねた。
# 画面には「◯月◯日時点」と出るので、**新しい方**を出さないと客に古い日付を見せることになる。
# (2026-08-14 独立レビュー指摘: 情報を変えたのに 8月4日 のままだった)
DETAILS_AS_OF = '2026年8月14日'  # 店舗情報の最終更新日 (公式取り込み 8/4 + 組合訂正 8/14)
